...
**The bot goes quiet while a job runs.** The daemon polls Telegram and runs its scheduled work in one sequential loop, so it answers nothing until the current job finishes. In practice that is about two minutes per hour — unnoticeable. The exception is the weekly re-validation, which runs the full battery: measured at 103 minutes on one shared ARM core, roughly half that on two. Once a week, the bot is unresponsive for that window and then catches up on whatever you sent; Telegram holds commands for 24 hours, so nothing is lost. No read command triggers this — `/summary` and `/details` read the stored result and answer instantly.
